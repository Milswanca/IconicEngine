...
bool FileUtils::DoesFileExist(const std::string& File)
{
    struct stat buffer;   
    return (stat (File.c_str(), &buffer) == 0); 
}

bool FileUtils::LoadFileToString(const std::string& File, std::string& Dest)
{
    std::ifstream f(File);
    if(f)
    {
...
std::string FileUtils::GetPathExtension(const std::string& File, bool bIncludeDot)
{
...
bool FileUtils::FindRelativePath(const std::string& BaseDir, const std::string& AbsolutePath, std::string& OutPath)
{
